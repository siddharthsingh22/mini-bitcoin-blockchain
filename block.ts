...
export class Block {
    constructor(public blockNumber?: number, public timeStamp?: number, public lastHash?: string, public hash?: string, public nonce?: number, public difficulty?: number, public data?: number[]) { }
    static genesis() { // DO not use arrow function else following error would come ---> 'this' cannot be referenced in a static property initializer
        ...
        return new this(0, 0, "-", "#", 0, 0, []); // this refers to class 'Block' itself
    }
    toString = () => {
        return `
             =======Block details======
    ...
    static nonceFunction(timeStamp: number, lastHash: string, data: number[], difficulty: number) { // finds hash of the block and the nonce
        ...
        let hash: string = "0";
        do {
            this.hashFunction(timeStamp, lastHash, data, nonce);
            nonce++;
        } while (hash.substring(0, difficulty) != "0".repeat(difficulty)) {
            hash = this.hashFunction(timeStamp, lastHash, data, nonce)
            nonce++;
            console.log(hash);
        }
        return { hash, nonce };
    ...
    static mineBlockFunction(lastBlock: any, data: number[]) {
        ...
        let difficulty = 2;
        let { hash, nonce } = this.nonceFunction(timeStamp, lastHash, data, difficulty);
        return new this(blockNumber, timeStamp, lastHash, hash, nonce, difficulty, data); // this refers to the class 'Block' itself 
    }
}
